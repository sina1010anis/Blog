    ...
    <div class="group-view-all-panel">
        ...
        <span class="w-50 fl-right  view-setup-admin-panel"
            ...
            <span class="w-100 fl-left">
                <h3 align="center" class="set-font color-b-600">Default Menu</h3>
                <span class="line fl-left"></span>
                @foreach($name_domain->menu as $menu)
                    <span @click="delete_item_menu('{{$menu->id}}' , '{{$name_domain->domain}}')" class="view-item-panel-admin" :id="'view-item-panel-admin'+{{$menu->id}}">
                        <a class="set-font color-b-700 w-100 pointer item-menu-view" align="center" dir="rtl">{{$menu->name}}<b></b></a>
                    </span>
                @endforeach
            </span>
        </span>
            ...
            <span class="w-100 fl-left">
                <h3 align="center" class="set-font color-b-600">New Menu</h3>
                <span class="line fl-left"></span>
                <form class="form-edit-blog" action="{{route('admin.new.menu' , ['dom' => $name_domain->domain])}}"
                      method="post">
                    @csrf
